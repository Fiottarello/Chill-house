...
function Prenota({ isAuth }) {
  ...
  return (
    <div className="container py-5">
      <h2 className="text-center mb-4">Prenota un soggiorno 📅</h2>
      <Card className="modern-card mx-auto" style={{ maxWidth: '500px' }}>
        <Form onSubmit={handleSubmit}>
          <Form.Group className="mb-3"><Form.Label>Arrivo</Form.Label><Form.Control type="date" required onChange={e => setForm({...form, arrivo: e.target.value})} /></Form.Group>
          <Form.Group className="mb-3"><Form.Label>Partenza</Form.Label><Form.Control type="date" required onChange={e => setForm({...form, partenza: e.target.value})} /></Form.Group>
          <Form.Group className="mb-3"><Form.Label>Numero ospiti</Form.Label><Form.Control type="number" min="1" value={form.numero_ospiti} onChange={e => setForm({...form, numero_ospiti: e.target.value})} /></Form.Group>
          <Form.Group className="mb-3"><Form.Label>Note</Form.Label><Form.Control as="textarea" onChange={e => setForm({...form, note: e.target.value})} /></Form.Group>
...
export const addUser = (u) =>
  openDb().then(db => new Promise((resolve, reject) => {
    const sql = `INSERT INTO user(name, surname, email, password, salt, phoneNumber, occupazione, da_quanto_ci_conosci, descrizione_simpatica, avatar_url, role) 
                 VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
    db.run(sql, [u.name, u.surname, u.email, u.password, u.salt, u.phoneNumber, u.occupazione, u.da_quanto_ci_conosci, u.descrizione_simpatica, u.avatar_url, u.role], function (err) {
      closeDb(db);
      if (err) return reject(err);
      resolve(this.lastID);
    });
...
app.post('/api/register', async (req, res) => {
  try {
    const { name, surname, email, password, phoneNumber, occupazione, da_quanto_ci_conosci, descrizione_simpatica, avatar_url } = req.body;
